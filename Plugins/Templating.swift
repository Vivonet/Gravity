...
extension Gravity {
	...
	@objc public class Templating: GravityPlugin {
		...
		public override func preprocessAttribute(var node: GravityNode, attribute: String, inout value: String) -> GravityResult {
			do { // <- fix this
				...
				var newValue: NSString = value as NSString
				...
				for match in matches.reverse() { // work from the back of the string so the ranges don't get clobbered
					...
					var keyPath = (value as NSString).substringWithRange(innerRange) // experimental
