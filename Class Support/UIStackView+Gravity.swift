...
extension UIStackView: GravityElement {
	...
	public func processElement(node: GravityNode) -> Bool {
		...
		// we need to ensure that our subviews are created by this point
		...
		// TODO: move this into the UIStackView handler
		// MARK: Content Compression Resistance
		let baseCompressionResistance = Float(750)
		var shrinkIndex = Dictionary<Int, GravityNode>()
		for childNode in node.childNodes {
...
//			guard let subview = 
		...
		var growIndex = Dictionary<Int, GravityNode>()
		for childNode in node.childNodes {
